...
function nav(){let n=document.getElementById('nav');if(!n||n.dataset.v80)return;n.dataset.v80='1';n.innerHTML=G.map(([g,x])=>'<div class="nav-group"><div class="nav-label">'+g+'</div>'+x.map(([r,i,t])=>'<button type="button" class="nav-item '+(active===r?'active':'')+'" data-v80="'+r+'"><span class="nav-icon">'+i+'</span><span>'+t+'</span></button>').join('')+'</div>').join('')}
...
function boot(){if(!document.getElementById('nav'))return setTimeout(boot,100);style();nav();new MutationObserver(nav).observe(document.getElementById('nav'),{childList:true});let h=location.hash.replace(/^#\//,'').replaceAll('/','.');if(P[h]){active=h;render()}}boot();
})();
