...
function editing(){if(window.OctopusStableActionRegistry?.editing?.())return true;const a=document.activeElement;if(a&&/^(INPUT|SELECT|TEXTAREA)$/.test(a.tagName))return true;return !!document.querySelector('#octopusRowEditor.open,.atw-modal,.air-modal,.prw-modal,.rvw-editor-top,.dpw-modal,.dsao-modal,.oar-modal,.gw3-modal,.business-modal,.ubw-layer,.wmk-modal')}
...
function check(){css();const r=route(),root=document.getElementById(ROOT);if(!root||editing())return;if(r!==lastRoute){lastRoute=r;routeChangedAt=Date.now();return}window.OctopusStableActionRegistry?.check?.();
 ...
 if(r==='system.templates'){const p=root.querySelector('.air-page');if(!p){recover('template workspace missing');return}showButtons(p,'.air-btn');return}
