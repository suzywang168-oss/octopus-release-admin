...
function editing(){if(window.OctopusStableActionRegistry?.editing?.())return true;const a=document.activeElement;if(a&&/^(INPUT|SELECT|TEXTAREA)$/.test(a.tagName))return true;return !!document.querySelector('#octopusRowEditor.open,.atw-modal,.prw-modal,.rvw-editor-top,.dpw-modal,.dsao-modal,.oar-modal,.business-modal')}
...
function clearState(root){['atw','prw','daw','pcw','route'].forEach(k=>{try{delete root.dataset[k]}catch{}})}
...
window.OctopusIntegrityGuard={check,editing,version:'4.6'};window.addEventListener('hashchange',()=>{lastRoute=route();routeChangedAt=Date.now();setTimeout(check,700)});window.addEventListener('pageshow',()=>setTimeout(check,700));window.addEventListener('octopus-language-change',()=>setTimeout(check,700));document.addEventListener('focusout',()=>setTimeout(check,260),true);document.addEventListener('visibilitychange',()=>{if(!document.hidden)setTimeout(check,700)});if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>setTimeout(check,700),{once:true});else setTimeout(check,700);setInterval(check,INTERVAL);
})();