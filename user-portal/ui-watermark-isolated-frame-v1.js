...
function render(){if(!active())return;style();title();const root=document.getElementById(ROOT);if(!root)return;let frame=root.querySelector(':scope > .'+FRAME_CLASS);if(!frame){root.replaceChildren();frame=document.createElement('iframe');frame.className='wm4-page '+FRAME_CLASS;frame.title='频道水印处理';frame.src='./watermark-standalone.html?v=202608171829#/release/watermark';frame.setAttribute('loading','eager');frame.setAttribute('allow','autoplay; clipboard-read; clipboard-write');root.appendChild(frame)}root.dataset.routeOwner=ROUTE}
...
window.OctopusWatermarkSingleWorkspace={render,ensure,version:'8.1-isolated-frame'};
