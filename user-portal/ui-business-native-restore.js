...
'use strict';
const ROOT='pageRoot';
const NATIVE_BUTTON_ROUTES=new Set(['system.templates','system.roles','release.review','release.distribution']);
...
function releaseContracts(){
 ...
 if(mi?.routes)removeFrom(mi.routes,'system.templates');
}
function cleanupNativeButtons(){
 ...
 if(r==='system.templates'){
  ...
  root.querySelector('.atw-page')?.removeAttribute('data-metrics-insight');
 }
}
...
window.addEventListener('hashchange',()=>{setTimeout(apply,40);setTimeout(apply,260)});
window.addEventListener('octopus-owned-route-change',()=>setTimeout(apply,60));
window.addEventListener('pageshow',()=>setTimeout(apply,120));
window.addEventListener('octopus-language-change',()=>setTimeout(apply,140));
document.addEventListener('focusout',()=>setTimeout(apply,180),true);
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>setTimeout(apply,180),{once:true});else setTimeout(apply,80);
window.OctopusBusinessNativeRestore={apply,version:'1.0'};
})();