...
function open(button,info){
 ...
 if(mode==='creative'){
   ...
   cards='<label class="ore-field"><label>匹配内部剧集</label><select data-launch-series><option>逆光心动</option><option>契约之后</option><option>她从雨夜归来</option></select></label><label class="ore-field"><label>视频版本</label><select><option>英语成片 V3</option><option>西语成片 V2</option><option>英语成片 V1</option></select></label><label class="ore-field"><label>目标地区 / 语种</label><select data-launch-region><option>北美 · 英语</option><option>拉美 · 西班牙语</option></select></label><label class="ore-field"><label>频道账号</label><select data-launch-channel><option>TK-US Drama</option><option>FB-Latina</option><option>YT-English</option></select></label><label class="ore-field"><label>测试预算</label><input value="$1,500"></label><label class="ore-field"><label>上线时间</label><input value="明天 18:00"></label><label class="ore-field full"><label>匹配依据与测试目标</label><textarea>外部素材近 7 天增速高，题材与内部剧集核心标签匹配；采用小流量测试，CTR 达到 7.5% 后扩大分发。</textarea></label>';
 ...
 const confirmLabel=mode==='approval'?'提交审核结论':mode==='api'?'保存密钥配置':mode==='launch'?'创建上线测试任务':mode==='candidate'?'加入候选片单':mode==='ingest'?'提交入库并开始上传':mode==='remove'?'确认移出':'确认操作';
