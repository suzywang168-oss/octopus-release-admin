...
const managed=()=>['release.review','system.assets','system.templates','system.roles'].includes(route());
...
#${ROOT} :is(.rvw-btn,.atw-btn,.prw-btn,.abs-btn){visibility:visible!important;opacity:1!important;pointer-events:auto!important}
#${ROOT} :is(.rvw-actions,.atw-tabs,.prw-tabs,.abs-actions){visibility:visible!important;opacity:1!important}
#${ROOT} .rvw-head,#${ROOT} .atw-head,#${ROOT} .prw-head{overflow:visible!important}
#${ROOT} .rvw-actions{display:flex!important;align-items:center!important;gap:8px!important}
`}
...
function check(){scheduled=false;css();const root=document.getElementById(ROOT);if(!root)return;const r=route();if(r==='release.review')review(root);else if(r==='system.assets')assets(root);else if(r==='system.templates')templates(root);else if(r==='system.roles')roles(root)}
...
window.OctopusStableActionRegistry={check,version:'3.1'};
