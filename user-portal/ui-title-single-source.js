...
function apply(){css();document.querySelectorAll('.oct-title-duplicate').forEach(x=>x.classList.remove('oct-title-duplicate'));const bar=document.querySelector('.ota-toolbar'),slot=document.getElementById(SLOT);if(!bar||!slot)return;const title=clean(slot.querySelector('h1')?.textContent);if(!title)return;
 [...bar.querySelectorAll('h1,h2,h3,strong')].forEach(h=>{if(slot.contains(h)||clean(h.textContent)!==title)return;(directChild(h,bar)||h).classList.add('oct-title-duplicate')});
...
window.addEventListener('hashchange',()=>{schedule(0);schedule(140)});window.addEventListener('pageshow',()=>schedule(80));window.addEventListener('octopus-language-change',()=>schedule(100));window.addEventListener('resize',()=>schedule(80));if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>schedule(160),{once:true});else schedule(160);setTimeout(()=>schedule(),700);window.OctopusTitleSingleSource={apply,version:'1.1'};
})();